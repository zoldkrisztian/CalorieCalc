...
public class RegistrationViewController {
	...
	public void setStage(Stage stage) {
		this.stage = stage;
		//this.stage.setOnHiding((o) -> main.showMainView());
	...
	public void pressRegistrationButton() {
		...
		if (verifyFields()) {
			...
			Gender gender = (genderComboBox.getSelectionModel().selectedIndexProperty().getValue() == 1) ? (gender = Gender.MALE) : (gender = Gender.FEMALE);

			Goal goal = (goalComboBox.getSelectionModel().selectedIndexProperty().getValue() == 1) ? (goal = Goal.KEEP_BODYWEIGHT) : (goal = Goal.LOSE_BODYWEIGHT);

			user = new User(userNameField.getText(), firstNameField.getText(), lastNameField.getText(), Double.valueOf(bodyheightField.getText()), Double.valueOf(bodyweightField.getText()), Integer.parseInt(ageField.getText()), gender, goal);
